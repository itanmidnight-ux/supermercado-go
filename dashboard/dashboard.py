...
#  ██║        ██║   ███████║███████║███████╗██║  ██║   ██║   ███████║
#  ╚═╝        ╚═╝   ╚══════╝╚══════╝╚══════╝╚═╝  ╚═╝   ╚═╝   ╚══════╝
#  dashboard.py — Panel de administración del servidor v4.0
#
#  14 módulos: Monitoreo · Ventas · Pedidos · Bot WhatsApp · Pagos · Empleados
#              · Ubicaciones · Conexiones · Datos · Marca · Dominio · Config
#              · Seguridad · Logs
#
#  Arquitectura:
#   - Sidebar lateral colapsable + área principal responsive (1200x800 min)
#   - Estilo claro empresarial con acentos de marca, crossfade nativo
#   - Gráficos Cairo dibujados a mano (barras, líneas, dona, sparklines)
#   - Pool de conexiones PostgreSQL con reconnect automático
#   - Rate limiting para endpoints sensibles
#   - Validación de entrada en todos los puntos de acceso
#
#  Seguridad implementada:
#   - Pool de conexiones con timeout y max_retries
#   - Rate limiting por IP (anti-fuerza bruta)
#   - Validación de inputs con regex
#   - Sanitización de queries SQL (parámetros parametrizados)
#   - Protección XSS en outputs
#   - Logging de auditoría para acciones sensibles
#
#  Se lanza desde deploy-linux.sh o directamente: python3 dashboard.py
# ================================================================================
import gi
gi.require_version('Gtk', '3.0')
from gi.repository import Gtk, Gdk, GLib, GdkPixbuf
import cairo
import subprocess, os, re, sys, datetime, secrets, json, base64, threading, math, time
import urllib.request, urllib.error
import shutil, platform, hashlib, hmac
from collections import defaultdict, deque
import psycopg2
from contextlib import contextmanager

# ══════════════════════════════════════════════════════════════════════════════
# SEGURIDAD — Pool de conexiones, Rate Limiting, Validación
# ══════════════════════════════════════════════════════════════════════════════

class PostgreSQLPool:
    """Pool de conexiones PostgreSQL con reconnect automático y thread-safety.

    Mejora significativa sobre crear/cerrar conexiones en cada query:
    - Reutiliza conexiones existentes (reduce overhead de handshake TCP)
    - Detecta conexiones muertas y reconecta automáticamente
    - Limita el número máximo de conexiones simultáneas
    - Timeout configurable para evitar conexiones colgadas
    """

    def __init__(self, min_conn=2, max_conn=10, timeout=5, max_retries=3):
        ...
        self._stats = {'created': 0, 'reused': 0, 'failed': 0, 'reconnected': 0}
    ...
    def _create_connection(self):
        ...
        try:
            ...
            if database_url:
                conn = psycopg2.connect(database_url, connect_timeout=self._timeout)
            else:
                conn = psycopg2.connect(
                    host=env_get('PG_HOST') or '127.0.0.1',
                    port=env_get('PG_PORT') or '5433',
            ...
            # Solo loguear el primer error, no spam
            if self._stats['failed'] <= 1:
                print(f'[pool] PostgreSQL no disponible: {e}', file=sys.stderr)
                print(f'[pool] El dashboard funcionará con datos vacíos hasta que PG esté disponible.', file=sys.stderr)
            return None
    ...
    def get_connection(self):
        ...
        with self._lock:
            ...
            if conn is None and self._active_count < self._max_conn:
                conn = self._create_connection()
                if conn:
                    self._active_count += 1

        if conn is None:
            # Pool agotado o falló la creación
            raise Exception("No hay conexiones disponibles en el pool")

        try:
            yield conn
        except Exception as e:
            # En caso de error, intentar reconexión una vez
            try:
                conn.close()
            except Exception:
                pass
            ...
            # Crear nueva conexión para el siguiente uso
...
class RateLimiter:
    ...
    Previene ataques de fuerza bruta y DDoS ligero:
    ...
    def __init__(self, max_requests=100, window_seconds=60, block_duration=300):
        self._max_requests = max_requests
        self._window = window_seconds
        self._block_duration = block_duration
        self._requests = defaultdict(deque)  # ip -> deque of timestamps
        self._blocked = {}  # ip -> block_until_timestamp
        self._lock = threading.Lock()
    ...
    def get_status(self, ip):
        """Retorna el estado actual de una IP."""
        with self._lock:
            now = time.time()
            self._cleanup(ip)
            return {
                ...
                'blocked': ip in self._blocked and now < self._blocked.get(ip, 0),
                'block_remaining': max(0, self._blocked.get(ip, 0) - now),
            }
...
class InputValidator:
    ...
    def validate_email(cls, email):
        ...
        email = cls.sanitize_string(email, cls.MAX_EMAIL_LENGTH)
    ...
    def validate_phone(cls, phone):
        ...
        phone = cls.sanitize_string(phone, cls.MAX_PHONE_LENGTH)
        if not cls.PHONE_RE.match(phone):
            return False, 'Formato de teléfono inválido (7-15 dígitos)'
        return True, phone
    ...
    def validate_date(cls, date_str):
        ...
        if not cls.DATE_RE.match(date_str):
            return False, 'Formato de fecha inválido (use YYYY-MM-DD)'
        try:
            datetime.datetime.strptime(date_str, '%Y-%m-%d')
            return True, date_str
        except ValueError:
            return False, 'Fecha inexistente'

    @classmethod
    def validate_hex_color(cls, color):
        """Valida un color hexadecimal."""
        if not color:
            return False, 'Color vacío'
        color = cls.sanitize_string(color, 7)
        if not cls.HEX_COLOR_RE.match(color):
            return False, 'Formato de color inválido (use #RRGGBB)'
        return True, color
    ...
    def validate_port(cls, port):
        """Valida un puerto TCP."""
        return cls.validate_integer(port, 1, 65535)

    @classmethod
    def safe_sql_string(cls, value):
        """Escapa un string para uso seguro en SQL (como fallback).

        IMPORTANTE: Se deben usar queries parametrizados siempre.
...
class SecurityAudit:
    """Sistema de auditoría de seguridad para el dashboard.

    Registra acciones sensibles para análisis forense:
    ...
    def __init__(self, log_file=None):
        self._log_file = log_file or os.path.join(
            ...
            'server', 'data', 'dashboard_audit.log'
        )
        self._lock = threading.Lock()

    def log_event(self, event_type, details, user='dashboard'):
        ...
        with self._lock:
            try:
                ...
                print(f'[audit] Error escribiendo log: {e}', file=sys.stderr)
    ...
    def log_config_change(self, key, old_value, new_value):
        ...
        safe_old = '***' if any(s in key.lower() for s in ['secret', 'password', 'key', 'token']) else old_value
        safe_new = '***' if any(s in key.lower() for s in ['secret', 'password', 'key', 'token']) else new_value
        self.log_event('config_change', {
            'key': key,
            'old_value': safe_old[:100],
            'new_value': safe_new[:100],
        })

    def log_ip_block(self, ip, action, reason=''):
        """Registra bloqueo/desbloqueo de IP."""
        self.log_event('ip_block', {
            'ip': ip,
            'action': action,
            'reason': reason,
        })

    def log_data_export(self, categories, format_type):
    ...
    def log_auth_attempt(self, success, username='', ip=''):
...
_rate_limiter = RateLimiter(max_requests=100, window_seconds=60, block_duration=300)
...
# Tema dark moderno con acentos de marca (verde #00B860 / naranja #FF8C00)
BG          = '#0f1117'   # fondo principal (casi negro)
SURFACE     = '#1a1d23'   # cards, contenido
SURFACE_2   = '#22262e'   # sidebar, hover, elevated
SURFACE_3   = '#2a2f38'   # active, pressed
BORDER      = '#333842'   # bordes sutiles
BORDER_SOFT = '#2a2f38'   # dividers
FG          = '#e4e6ea'   # texto principal (casi blanco)
FG_MUTED    = '#9ca3af'   # texto secundario
FG_DIM      = '#6b7280'   # labels, texto terciario
ACCENT      = '#3b82f6'   # azul acciones primarias
BRAND       = '#00B860'   # verde marca (highlights)
BRAND_DARK  = '#059669'   # verde oscuro
SUCCESS     = '#10b981'   # OK, activo
WARNING     = '#f59e0b'   # advertencias
DANGER      = '#ef4444'   # errores, crítico
INFO        = '#60a5fa'   # info charts
...
CSS = f"""
/* ═══════════════════════════════════════════════════════════════════════
   SUPERMERCADOS GO — Dark Theme Profesional v4.0
   Tema oscuro con acentos de marca (verde #00B860 / naranja #FF8C00)
   ═══════════════════════════════════════════════════════════════════════ */
...
/* ─── Content area ───────────────────────────────── */
.content {{ background-color: {BG}; padding: 22px 24px; }}
...
.day-bar-bg {{ background-color: {SURFACE_3}; border-radius: 4px; }}
.day-bar-fg {{ background: linear-gradient(90deg, {BRAND} 0%, {SUCCESS} 100%); border-radius: 4px; }}
...
.pill-success {{ background: rgba(16,185,129,0.15); color: {SUCCESS}; border: 1px solid rgba(16,185,129,0.3); }}
.pill-warning {{ background: rgba(245,158,11,0.15); color: {WARNING}; border: 1px solid rgba(245,158,11,0.3); }}
.pill-danger  {{ background: rgba(239,68,68,0.15); color: {DANGER};  border: 1px solid rgba(239,68,68,0.3); }}
.pill-muted   {{ background: {SURFACE_3};            color: {FG_MUTED}; border: 1px solid {BORDER}; }}
.pill-info    {{ background: rgba(96,165,250,0.15); color: {INFO};    border: 1px solid rgba(96,165,250,0.3); }}
.pill-brand   {{ background: rgba(0,184,96,0.15);   color: {BRAND};   border: 1px solid rgba(0,184,96,0.3); }}
...
.btn-primary:hover {{ background: linear-gradient(135deg, #2563eb 0%, {ACCENT} 100%); }}
.btn-primary:active {{ background: #1d4ed8; }}
.btn-brand {{
    background: linear-gradient(135deg, {BRAND} 0%, {SUCCESS} 100%);
    color: {BG};
    border: 1px solid {BRAND};
}}
.btn-brand:hover {{ background: linear-gradient(135deg, {SUCCESS} 0%, {BRAND} 100%); }}
.btn-warn {{
    background-color: transparent;
    color: {WARNING};
    border: 1px solid {WARNING};
}}
.btn-warn:hover {{ background-color: rgba(245,158,11,0.1); }}
.btn-danger {{
    background-color: transparent;
    color: {DANGER};
...
entry:disabled {{ color: {FG_DIM}; background: {SURFACE_3}; }}
...
treeview row:selected {{ background-color: rgba(59,130,246,0.2); color: {FG}; }}
treeview row:hover {{ background-color: rgba(59,130,246,0.08); }}

/* ─── Textview (logs, security) ──────────────────── */
textview {{ background-color: {SURFACE}; }}
textview text {{ background-color: {SURFACE}; color: {FG_MUTED}; }}
textview selection {{ background-color: rgba(59,130,246,0.3); }}
...
.preset-selected {{ border: 2px solid {BRAND}; box-shadow: 0 0 8px rgba(0,184,96,0.3); }}
...
def _resolve_api_base():
    """Resuelve la URL base del API: lee PORT del .env (si existe) → 3777 por defecto.
    Autocontenida para poder llamarse al inicio del módulo antes que env_get()."""
    port = os.environ.get('PORT') or '3777'
...
def pg_connect():
    """Conexion a Postgres -- pool de conexiones con reconnect automático.
    Usa las mismas env vars que server/src/db/database.js."""
    # Se mantiene por compatibilidad, pero se recomienda usar _pool.get_connection()
    database_url = env_get('DATABASE_URL')
    if database_url:
...
# Pool global de conexiones (inicializado después de env_get disponible)
_pool = None
...
def _get_pool():
    ...
    global _pool
    if _pool is None:
        _pool = PostgreSQLPool(min_conn=2, max_conn=10, timeout=5, max_retries=3)
    return _pool
...
def query(sql, params=()):
    """Query Postgres de solo lectura con pool de conexiones.

    Características de seguridad:
    - Usa pool de conexiones (reutilización, reconnect automático)
    - Solo lectura (readonly=True)
    - Queries parametrizadas (previene SQL injection)
    - Timeout para evitar queries colgadas

    Devuelve lista de tuplas o [] si falla.
    """
    ...
    for pattern in dangerous_patterns:
        if sql_upper.startswith(pattern):
            _security_audit.log_event('blocked_query', {'sql': sql[:200], 'reason': 'write_in_readonly'})
            return []
    ...
    try:
        ...
        # No spamear errores - solo el primer error
        pool = _get_pool()
        if pool._stats['failed'] <= 1:
            print(f'[db] PostgreSQL no disponible, mostrando datos vacíos', file=sys.stderr)
        return []


def db_write(sql, params=()):
    """Escribe en Postgres con validación de seguridad.
    ...
    Devuelve True/False.
    """
    # Validar que sea una operación de escritura
    sql_upper = sql.upper().strip()
    write_operations = ['INSERT ', 'UPDATE ', 'DELETE ', 'UPSERT ']
    is_write = any(sql_upper.startswith(op) for op in write_operations)

    if not is_write and not sql_upper.startswith('SELECT'):
        _security_audit.log_event('blocked_query', {'sql': sql[:200], 'reason': 'unknown_operation'})
        return False

    # Loggear cambios en tablas sensibles
    sensitive_tables = ['users', 'settings', 'blocked_ips', 'security_alerts']
    for table in sensitive_tables:
        if table in sql.lower():
            _security_audit.log_event('sensitive_write', {'table': table, 'sql': sql[:200]})
            break
    ...
    try:
        ...
        pool = _get_pool()
        if pool._stats['failed'] <= 1:
            print(f'[db] PostgreSQL no disponible para escritura', file=sys.stderr)
        return False


def setting_get(key, default=''):
    """Obtiene un valor de settings con validación."""
    # Validar key
    if not key or not isinstance(key, str):
        return default
    key = _input_validator.sanitize_string(key, 100)
    if not key:
        return default
...
def setting_set(key, value):
    """Establece un valor en settings con validación y auditoría."""
    # Validar key
    if not key or not isinstance(key, str):
        return False
    key = _input_validator.sanitize_string(key, 100)
    ...
    if value is not None and isinstance(value, str):
        value = _input_validator.sanitize_string(value, 5000)

    # Auditoría para cambios sensibles
    _security_audit.log_config_change(key, '(previous)', str(value)[:100] if value else '')
...
def _get_admin_token():
    ...
    El dashboard asume que existe un usuario admin configurado en
    DASHBOARD_ADMIN_USER / DASHBOARD_ADMIN_PASS del .env (default: admin@supermercado.go)."""
    import time
    now = time.time()
    if _ADMIN_TOKEN['value'] and now < _ADMIN_TOKEN['expires']:
        return _ADMIN_TOKEN['value']
    user = env_get('DASHBOARD_ADMIN_USER') or 'admin@supermercado.go'
    pw   = env_get('DASHBOARD_ADMIN_PASS') or 'admin123'
    url  = API_BASE + '/api/auth/login'
    body = json.dumps({'email': user, 'password': pw}).encode('utf-8')
    req  = urllib.request.Request(url, data=body, method='POST', headers={
        'Content-Type': 'application/json'
    })
    try:
...
def _get_admin_user_id():
    """Extrae el user ID del JWT admin (decodifica el payload base64)."""
...
class OrdersModule:
    ...
    def _action(self, oid, action):
        ...
        Mapea acciones del dashboard a los endpoints del server:
          claim    → POST /api/orders/:id/assign (asigna admin como worker)
          unclaim  → PUT  /api/orders/:id/status {status:'ready'}
          en_camino→ PUT  /api/orders/:id/status {status:'in_transit'}
          deliver  → PUT  /api/orders/:id/status {status:'delivered'}
        """
        STATUS_MAP = {
            'claim':     ('POST', f'/api/orders/{oid}/assign',  {'worker_id': _get_admin_user_id()}),
            'unclaim':   ('PUT',  f'/api/orders/{oid}/status',  {'status': 'ready'}),
            'en_camino': ('PUT',  f'/api/orders/{oid}/status',  {'status': 'in_transit'}),
            'deliver':   ('PUT',  f'/api/orders/{oid}/status',  {'status': 'delivered'}),
        }
        method, path, body = STATUS_MAP.get(action, ('PUT', f'/api/orders/{oid}/status', {'status': action}))
        if method == 'POST':
            fn = lambda: http_post(path, body)
        else:
            fn = lambda: http_put(path, body)
        run_in_background(fn, lambda result: self._on_action_done(oid, action, result))
    ...
    def _action_cancel(self, oid):
        ...
        if dialog.run() == Gtk.ResponseType.OK:
            ...
            if reason:
                run_in_background(lambda: http_post(f'/api/orders/{oid}/cancel', {'reason': reason}),
...
class ConfigModule:
    ...
    def refresh(self):
        ...
        pg_target = f"{env_get('PG_USER') or 'pedidosbot'}@{env_get('PG_HOST') or '127.0.0.1'}:{env_get('PG_PORT') or '5433'}/{env_get('PG_DATABASE') or 'supermercado'}"
...
class DashboardWindow(Gtk.ApplicationWindow):
    ...
    que intercambia entre los 14 módulos disponibles.

    Características:
    - Pool de conexiones PostgreSQL con reconnect automático
    - Rate limiting para endpoints sensibles
    - Validación de entrada en todos los puntos de acceso
    - Auditoría de seguridad para acciones críticas
    - CSS mejorado con sombras y transiciones suaves
    - Soporte para 14 módulos de administración
    """

    def __init__(self, app):
        super().__init__(application=app, title='Supermercados Go — Panel de Administración v4.0')
        ...
        header.set_title('🛒 Supermercados Go')
        header.set_subtitle('Panel de Administración v4.0 — Seguro y Moderno')
        ...
        ver = Gtk.Label(label='v4.0 · PostgreSQL · GTK3 nativo')
        ...
        sec_label = Gtk.Label(label='🔒 Pool de conexiones activo')
        sec_label.get_style_context().add_class('label-dim')
        sec_label.set_xalign(0)
        footer.pack_start(sec_label, False, False, 0)
...
def main():
    app = Gtk.Application(application_id='com.supermercadosgo.dashboard.v4',
                          flags=0)
